...
class Devices:
    """Integration driver configuration class. Manages all configured Lumagen devices."""
    ...
    def __init__(
        ...
        self._config: list[LumagenInfo] = []
        self._add_handler = add_handler
        self._remove_handler = remove_handler

        self.load()

    def add(self, lumagen: LumagenInfo) -> None:
        """Add a new configured Lumagen device, ignoring duplicates by ID."""
        if any(d.id == lumagen.id for d in self._config):
            _LOG.warning("Device with id '%s' already exists.", lumagen.id)
            return
        self._config.append(lumagen)
        if self._add_handler:
            self._add_handler(lumagen)
        self.store()
        _LOG.info("Device with id '%s' added and stored.", lumagen.id)
    ...
    def get(self, device_id: str) -> LumagenInfo | None:
    ...
    def update(self, updated: LumagenInfo) -> bool:
    ...
    def _write_config_file(self, data: list[LumagenInfo]) -> None:
    ...
    def load(self) -> bool:
        ...
        try:
            ...
            for item in data:
                if not all(k in item for k in ("id", "name", "address", "port")):
                    _LOG.warning("Skipping invalid config item: %s",  item)
                    continue
                try:
                    lumagen = LumagenInfo(**item)
            ...
            _LOG.info("No config file found at %s. Starting with an empty configuration.", self._cfg_file_path)
    ...
    def __iter__(self) -> Iterator[LumagenInfo]:
